...
from consts.nerve_props import DENDRITE_TYPE, PART_PROPS_DTYPE, PART_PROPS_MATRIX, TYPE, PART_PROPS_KEYS_MAP, RELEASE_TYPE, PART_PROPS, DYNAMIC_PROP_NAMES, STATIC_PROP_NAMES, SPINE_EXINFO, POSTERIOR_SYNAPSE_EXINFO
from consts.write_cortex import is_can_write, WRITE_PART_PROPS
from consts.nerve_params import ATP, PRE_LTP_SUM_BASE, DIED_LTP_THRESHOLD, POST_SYNAPSE_RESOURCE_SUM, POSTERIOR_SYNAPSE_TRANSMITTER_SUM, SPINE_SUM_ON_A_DENDRITE, STANDARD_PREDICT_EXCITE, ABSTRACT_SYNAPSE_RP
from util import print_exe_timecost, exchange_slice_to_inds
from experiments import REGION, form_init_nerve, form_synapse, experiment_enviroment, SOMA_SLICE_MAP
from utils.write_n_read_cortex import Write_n_read_cortex
from experiments.util import get_soma_inds
from experiments.mnist.experiment_enviroment.run_env.mock_globals import APPEAR_NERVE_SUFFIXs
...
class Cortex():
    ...
    def init_tick(self):
        ...
        # self.save_posterior_tick = None

    def form_debug_circuit(self):
        from experiments.form_nerve.debug_circuit import form_data
        current_mock_data = self.controller.input_mocker.mock_data_loader.current_data
        sequence_ind = int(current_mock_data['feature_name'].split('_')[3])
        mnist_form_data = form_data[
            current_mock_data['mnist_name']][sequence_ind]
        num, *features = mnist_form_data
        features = [[
            feature_prop for feature_prop in feature_props
            if feature_prop[1] != None
        ] for feature_props in features]
        max_LTP = np.power(
            STANDARD_PREDICT_EXCITE, 1 / len(features)
        ) / POSTERIOR_SYNAPSE_TRANSMITTER_SUM * POST_SYNAPSE_RESOURCE_SUM
        record_father_ind = None
        record_feature_start_ind = None
        spine_ind = None
        num_sign = np.sign(num)
        num = abs(int(num))

        for feature_ind in np.arange(len(features))[::-1]:
            feature_props = features[feature_ind]
            record_father_ind = record_feature_start_ind
            transmitter_release_sum = -ABSTRACT_SYNAPSE_RP / len(feature_props)

            if record_father_ind != None:
                father_ind = record_father_ind

            elif num_sign == -1:
                # 在抑制性树突棘上建立回路
                all_spine_inds = self.get_all_spine_inds(num,
                                                         is_inhibit_spine=True)
                spine_ind = father_ind = all_spine_inds[self.cortex[
                    SPINE_EXINFO['is_form_synapse']][all_spine_inds] == 0][0]
                self.cortex[SPINE_EXINFO['is_form_synapse']][father_ind] = 1
                self.cortex[SPINE_EXINFO['parents_info']][father_ind] = float(
                    current_mock_data['mnist_name'].replace('_', '.'))
                ...
                self.cortex[SPINE_EXINFO['is_form_synapse']][father_ind] = 1
                self.cortex[SPINE_EXINFO['parents_info']][father_ind] = float(
                    current_mock_data['mnist_name'].replace('_', '.') + '1')

            is_last_feture = feature_ind == len(features) - 1

            for prop_ind, (prop, value) in enumerate(feature_props):
                new_nerve_ind = self.controller.form_circuit_between_nerves(
                    get_soma_inds(
                        f'属性-{prop}',
                        f'{value}-{"群体编码出现" if is_last_feture else "群体编码消失"}_A{ ("前馈预测" if prop_ind==0 else "易化前馈预测") if is_last_feture else ("stp预测" if prop_ind==0 else "易化stp预测")}'
                    ), father_ind)[0]
                self.record_circuit_on_a_spine(spine_ind, new_nerve_ind,
                                               feature_ind)
                self.cortex['LTP'][
                    new_nerve_ind] = max_LTP if prop_ind == 0 else 1
                self.cortex[POSTERIOR_SYNAPSE_EXINFO['size']][
                    new_nerve_ind] = transmitter_release_sum
                record_father_ind = new_nerve_ind
                father_ind = record_father_ind
                self.cortex['RP'][new_nerve_ind] = ABSTRACT_SYNAPSE_RP
                if prop_ind == 0:
                    record_feature_start_ind = record_father_ind
                else:
                    self.cortex['transmitter_release_sum'][
                        new_nerve_ind] = -ABSTRACT_SYNAPSE_RP

        self.write_cortex('form_debug_circuit')

    def form_debug_circuit_with_prop_value_map_list(self, prop_value_map_list):
        current_mock_data = self.controller.input_mocker.mock_data_loader.current_data
        num = int(current_mock_data['mnist_name'].split('_')[0])
        feature_sum = len(prop_value_map_list)
        max_LTP = np.power(
            STANDARD_PREDICT_EXCITE, 1 / feature_sum
        ) / POSTERIOR_SYNAPSE_TRANSMITTER_SUM * POST_SYNAPSE_RESOURCE_SUM
        record_father_ind = None
        record_feature_start_ind = None
        spine_ind = None

        for feature_ind in np.arange(feature_sum)[::-1]:
            prop_value_map = prop_value_map_list[feature_ind]
            prop_sum = len(prop_value_map)
            record_father_ind = record_feature_start_ind
            transmitter_release_sum = -ABSTRACT_SYNAPSE_RP / prop_sum
            is_last_feture = feature_ind == feature_sum - 1
            ...
            if record_father_ind != None:
                ...
                # 在兴奋性树突棘上建立回路
                all_spine_inds = self.get_all_spine_inds(num)
                father_ind = spine_ind = all_spine_inds[self.cortex[
                    SPINE_EXINFO['is_form_synapse']][all_spine_inds] == 0][0]
                self.cortex[SPINE_EXINFO['is_form_synapse']][father_ind] = 1
                self.cortex[SPINE_EXINFO['parents_info']][father_ind] = float(
                    current_mock_data['mnist_name'].replace('_', '.') + '1')

            for prop_ind, (prop, value) in enumerate(prop_value_map.items()):
                mother_inds = get_soma_inds(
                    f'属性-{prop}',
                    f'{prop}{value}-{"群体编码出现" if is_last_feture else "群体编码消失"}_A{ ("前馈预测" if prop_ind==0 else "易化前馈预测") if is_last_feture else ("stp预测" if prop_ind==0 else "易化stp预测")}'
                )

                new_nerve_ind = self.controller.form_circuit_between_nerves(
                    mother_inds, father_ind)[0]
                self.record_circuit_on_a_spine(spine_ind, new_nerve_ind,
                                               feature_ind)
                self.cortex['LTP'][
                    new_nerve_ind] = max_LTP if prop_ind == 0 else 1
        ...
        self.write_cortex('form_debug_circuit_with_prop_value_map_list')

    def record_circuit_on_a_spine(self, spine_ind, nerve_ind, feature_ind):
        self.spine_circuit_map[spine_ind] = self.spine_circuit_map.get(
            spine_ind, {})
        self.spine_circuit_map[spine_ind][
            feature_ind] = self.spine_circuit_map[spine_ind].get(
                feature_ind, [])
    ...
    def reset_cortex_props_to_initial_state(self,
                                            reset_inds=None,
                                            exclude_props=[],
                                            include_props=[],
                                            reset_force=0):
        ...
        if len(include_props) > 0:
            ...
            reset_prop_names = [
                prop_name for prop_name in reset_prop_names
    ...
    def reset_cortex_single_prop_to_initial_state(self,
                                                  reset_inds,
                                                  prop_name,
                                                  reset_force=0):
        cortex_slice = np.asarray(reset_inds) if reset_inds else slice(
            self.new_ind_start)

        if prop_name == 'excite':

            is_can_reset_mask = self.cortex['self_synapse'][
                cortex_slice] <= reset_force

            if reset_inds:
                self.cortex['excite'][
                    cortex_slice[is_can_reset_mask]] = self.cortex['RP'][
                        cortex_slice[is_can_reset_mask]]

            else:
                self.cortex['excite'][cortex_slice][
                    is_can_reset_mask] = self.cortex['RP'][cortex_slice][
                        is_can_reset_mask]
            ...
            if unique_vals.size == 1:
                ...
                self.cortex[prop_name][cortex_slice] = np.asarray(
                    PART_PROPS[prop_name][1:])[self.cortex['type']
                                               [cortex_slice]]
    ...
    def load_cortex(self):
        ...
        self.new_ind_start = write_n_read_cortex.cortex_info[
            'cortex_slice_stop']
        ...
        for region_info in REGION.values():
            has_no_static_feature_nerves = [
                neuron for neuron in region_info['neurons'].values() if len([
                    feature_name
        ...
        # 加载marker map信息
        self.nerve_marker_map = write_n_read_cortex.cortex_info[
            'nerve_marker_map']
        self.update_new_marker_start()
    ...
    # def record_spine_marker_sypanses(self, cortex_info=None):
    #     if cortex_info:
    #         self.spine_marker_synapse_cortex_slice = slice(
    #             cortex_info['cortex_static_part_slice_stop'],
    #             cortex_info['spine_marker_synapse_cortex_slice_stop'],
    #         )
    #         self.new_spine_marker_synapse_ind_start = cortex_info[
    #             'new_spine_marker_synapse_ind_start']
    #     # else:
    #     #     self.cortex_static_nerve_slice = slice(
    #     #         self.cortex_static_part_slice.stop, self.new_ind_start)
    ...
    def init_static_parts(self):
        cortex = self.cortex
        self.col_no_start = LEN['col_no']  # 柱号设为三位数 从100开始
        self.new_ind_start += 1  # 将第0个nerve作为空nerve

        for region_name in REGION.keys():
            region_info = REGION[region_name]
            col_sum, neuron_sum, matrix_range, neuron_inds, col_nos, region_row_nos, region_hyper_col_nos, hyper_col_ind, mini_col_ind, neuron_nos, mother_neuron_nos, markers, neuron_in_pic_mask, pre_inds, post_inds, soma_inds = self.get_init_soma_params(
                region_info, self.col_no_start)
            self.new_ind_start += neuron_sum
            # 属性设置相互依赖，不要随意调整顺序
            self.set_soma_base_props(matrix_range, region_info,
                                     neuron_in_pic_mask)
            ...
            self.set_soma_features(matrix_range, region_info, col_sum,
                                   neuron_in_pic_mask)
    ...
    def get_init_soma_params(self, region_info, col_no_start):
        ...
        for neuron_name, neuron_info in region_info['neurons'].items():
            ...
            col_nos.extend(
                np.arange(col_sum)[neuron_in_pic_mask] + col_no_start)
            region_row_nos.extend(
                np.arange(region_row_sum).repeat(mini_col_sum_per_row)
                [neuron_in_pic_mask] + 1)
            region_hyper_col_nos.extend(
                np.tile(
                    np.arange(hyper_col_sum_per_row).repeat(
                        mini_col_sum_in_hyper_col), region_row_sum)
                [neuron_in_pic_mask] + 1)
            hyper_col_inds.extend(
                np.arange(region_info['hyper_col_sum'])
                [neuron_in_pic_mask].repeat(mini_col_sum_in_hyper_col))
            mini_col_inds.extend(
                np.tile(np.arange(mini_col_sum_in_hyper_col), hyper_col_sum)
                [neuron_in_pic_mask] + 1)
            neuron_nos.extend(
                np.tile([neuron_info['neuron_no']],
                        col_sum)[neuron_in_pic_mask])
            mother_neuron_nos.extend(
                np.tile([neuron_info['mother_neuron_no']],
                        col_sum)[neuron_in_pic_mask])
            markers.extend(
                np.tile([neuron_info['marker']], col_sum)[neuron_in_pic_mask])
            pre_inds.extend(
                exchange_slice_to_inds(SOMA_SLICE_MAP[region_name][
                    neuron_info['pre_name']]['cortex_slice']) if neuron_info.
                get('pre_name') else np.full(neuron_sum, -1))
            post_inds.extend(
                exchange_slice_to_inds(SOMA_SLICE_MAP[region_name][
                    neuron_info['post_name']]['cortex_slice']) if neuron_info.
                get('post_name') else np.full(neuron_sum, -1))
            soma_inds.extend(
                exchange_slice_to_inds(SOMA_SLICE_MAP[region_name][
                    neuron_info['soma_name']]['cortex_slice']) if neuron_info.
                get('soma_name') else np.full(neuron_sum, -1))

        matrix_range = slice(self.new_ind_start,
                             self.new_ind_start + all_neuron_sum)
        neuron_inds = np.arange(matrix_range.start, matrix_range.stop)
        return col_sum, all_neuron_sum, matrix_range, neuron_inds, col_nos, region_row_nos, region_hyper_col_nos, hyper_col_inds, mini_col_inds, neuron_nos, mother_neuron_nos, markers, all_neuron_in_pic_mask, pre_inds, post_inds, soma_inds

    def set_soma_base_props(self, matrix_range, region_info,
                            neuron_in_pic_mask):
    ...
    def set_soma_features(self, matrix_range, region_info, col_sum,
                          neuron_in_pic_mask):
        reset_map = {
            prop_name: np.repeat([
        ...
        for prop_name, prop_vals in reset_map.items():
            ...
            self.cortex[prop_name][matrix_range][set_mask] = prop_vals[
                set_mask]

    def init_static_nerves(self):
        for make_new_nerve_packs in form_init_nerve:
            if not callable(make_new_nerve_packs): continue
    ...
    def add_marker_remain(self, add_inds):
        ...
        self.cortex['dopamine_remain'][add_inds] = self.cortex[
            'marker_remain'][add_inds]

    def update_all_marker_inds(self):
        # marker_remain大于1的突触才算有效突触，否则就只是没有计算功能的丝足
        self.all_marker_inds = np.where(self.cortex['marker_remain'] > 1)[0]

    def is_can_form_synapse(self):
        return CORTEX_OPTS[
            'enable_posterior_form'] == 1 and self.controller.is_can_form_synapse(
            )

    def is_can_activate_dendritic_spine(self):
        current_mock_data = self.controller.input_mocker.mock_data_loader.current_data
        return current_mock_data[
            'play_direction'] == 'backward' and self.controller.input_mocker.get_is_last_feature(
            )
        # return self.controller.mock_props.nowa_feature_exinfo[
        #     'play_direction'] == 'backward' and self.controller.mock_props.get_is_last_feature(
        #     )
    ...
    def activate_dendritic_spine(self):
        ...
        if len(active_spine_inds) > 0:
            # 只活跃最活跃的那个树突棘
            active_spine_ind = active_spine_inds[np.argmax(
                self.get_spine_active_with_max_circuit_length(
                    active_spine_inds))]

            self.mark_spine_is_form_synapse(active_spine_ind)

            # 如果最活跃的棘已经存在稳定回路，则不赋予marker_remain进行强化
            if self.get_stable_spine_mask(np.array([active_spine_ind
                                                    ]))[0] == False:
    ...
    def mark_spine_is_form_synapse(self, spine_ind):
        self.cortex[SPINE_EXINFO['is_form_synapse']][
            self.get_all_spine_inds()] = 0
    ...
    def set_active_dendritic_spine_marker_remain(self,
        ...
        if is_new_spine:
            self.cortex['marker_remain'][
                activate_dendritic_spine_ind] = MAX_MARKER_REMAIN

        else:
            max_synapse_active_spine_force = np.max(
                self.cortex['spine_active'][activate_dendritic_spine_ind])
            self.cortex['marker_remain'][
                activate_dendritic_spine_ind] = MAX_MARKER_REMAIN * self.cortex[
                    'spine_active'][
                        activate_dendritic_spine_ind] / max_synapse_active_spine_force

    def get_new_active_dendritic_spine_ind(self):
        all_spine_inds_of_nowa_num = self.get_spine_inds_of_nowa_num()
        return all_spine_inds_of_nowa_num[self.cortex['max_circuit_length']
                                          [all_spine_inds_of_nowa_num] == 0][0]

    def get_active_spine_inds_of_nowa_num(self):
        all_spine_inds_of_nowa_num = self.get_spine_inds_of_nowa_num()
        return all_spine_inds_of_nowa_num[self.get_active_spine_mask(
            all_spine_inds_of_nowa_num)]

    def get_active_spine_inds_of_all_num(self):
        all_spine_inds_of_all_num = self.get_all_spine_inds()
        return all_spine_inds_of_all_num[self.get_active_spine_mask(
            all_spine_inds_of_all_num)]

    def get_active_spine_mask(self, spine_inds):
        MIN_SPINE_ACTIVE_RATIO_THRESHOLD = .25
        ...
        spine_resource_limit_of_each_synapse[
            stable_spine_mask] = self.get_resource_limit_of_spine(
                spine_inds[stable_spine_mask])
        return (self.cortex['spine_active'][spine_inds] /
                spine_resource_limit_of_each_synapse
    ...
    def get_spine_active_with_max_circuit_length(self, spine_inds):
        spine_resource_limit_of_each_synapse = self.get_resource_limit_of_spine(
            spine_inds)
        return self.cortex['spine_active'][
            spine_inds] / spine_resource_limit_of_each_synapse * self.cortex[
                'max_circuit_length'][spine_inds]
            ...
            tuple([
                get_soma_inds('数字', [
                    ...
                    for i in range(SPINE_SUM_ON_A_DENDRITE)
                              ...
                              hyper_col_inds_in_region=num) for num in nums
            ])).astype(int)
    ...
    def exe_make_new_nerve_packs(self, make_new_nerve_packs):
        ...
        new_nerve_slice, existed_nerve_inds, *is_loop = make_new_nerve_packs(
            self)
        ...
        if is_new_form or is_new_firm:
            self.used_parent_marker_inds.update(
                self.cortex['pre_ind'][new_nerve_slice])
            self.used_parent_marker_inds.update(
    ...
    def get_active_prop_inds(self):
        ...
        return cortex['ind'][POPU_ABSTRACT_APPEAR_INDS][(
            ...
            cortex['RP'][POPU_ABSTRACT_APPEAR_INDS]) == ABSTRACT_EXCITE]
    ...
    def record_has_marker_remain_and_stable_spine_parents_info(self):
        has_marker_remain_and_stable_spine_inds = self.get_has_marker_remain_and_stable_spine_inds_of_nowa_num(
        )
        if not len(has_marker_remain_and_stable_spine_inds): return

        self.cortex[SPINE_EXINFO['parents_info']][
            has_marker_remain_and_stable_spine_inds] = self.tick

        nowa_mnist_name = self.controller.mock_props.nowa_mnist_name.replace(
            '_', '0')

        self.cortex[SPINE_EXINFO['parents_info']][
            has_marker_remain_and_stable_spine_inds] = self.cortex[
                SPINE_EXINFO['parents_info']][
                    has_marker_remain_and_stable_spine_inds] * math.pow(
                        10,
                        len(nowa_mnist_name) + 1) + int(nowa_mnist_name)

        if self.controller.is_mocking_intersection_features():
            largest_spine_ind = str(self.controller.mock_props.
                                    nowa_feature_exinfo['largest_spine_ind'])
            self.cortex[SPINE_EXINFO['parents_info']][
                has_marker_remain_and_stable_spine_inds] = self.cortex[
                    SPINE_EXINFO['parents_info']][
                        has_marker_remain_and_stable_spine_inds] * math.pow(
                            10,
                            len(largest_spine_ind) +
                            1) + int(largest_spine_ind)
    ...
    def has_marker_remain_and_stable_spine_inherit_size_form_parent(self):
        if self.controller.is_mocking_intersection_features():
            ...
            self.cortex[SPINE_EXINFO['size']][
                has_marker_remain_and_stable_spine_inds] = self.cortex[
                    SPINE_EXINFO['size']][largest_spine_ind]
    ...
    def get_form_or_firm_synapse_inds_on_stable_spines(self):
        ...
        stable_spine_mask = self.get_stable_spine_mask(
            ...
            [form_or_firm_synapse_inds].astype(int))
    ...
    def get_has_synapse_but_unactive_spine_inds(self):
        ...
        return all_spine_inds_of_nowa_num[
            ...
            (self.cortex['max_circuit_length'][all_spine_inds_of_nowa_num] >
             0)]
    ...
    def init_spine_min_excite(self):
        self.cortex[SPINE_EXINFO['min_excite']][
            self.get_all_spine_inds()] = -np.inf
    ...
    def spike(self):
        ...
        # 树突自突触兴奋要在soma从其他地方获得兴奋前，也就是循环开始前执行，这样才能保证这时从soma获得的兴奋是它的自突触兴奋
    ...
    def spike_soma(self):
        ...
        to_inds = self.cortex['ind'][cortex_slice][
            ...
            (self.cortex['is_active'][self.cortex['pre_ind'][cortex_slice]]
             == 1)]

        spike_excite = self.cortex['tick_spike_times'][self.cortex['pre_ind'][
            to_inds]] * self.cortex['transmitter_release_sum'][
                self.cortex['pre_ind'][to_inds]]
        ...
        self.vary_axon_end_inds.update(
            np.compress(self.cortex['type'][to_inds] == TYPE['axon_end'],
                        to_inds))
        self.vary_soma_inds.clear()

        # 轴突末梢的状态更新后 它的突触前轴突末端也要参与后续计算
        self.cortex['bool_util'][cortex_slice] = False
        self.cortex['bool_util'][to_inds] = True
        vary_axon_end_inds = slice(self.cortex_static_part_slice.stop,
                                   self.new_ind_start)
        update_vary_axon_end_inds = np.compress(
            self.cortex['bool_util'][self.cortex['post_ind']
                                     [vary_axon_end_inds]],
    ...
    def get_reach_step_length_inds(self, nerve_inds):
        ...
        reach_step_length_mask = self.cortex['current_step'][
            nerve_inds] >= self.cortex['step_length'][nerve_inds]
        return nerve_inds[reach_step_length_mask], nerve_inds[
            ~reach_step_length_mask], reach_step_length_mask

    def update_axon_current_step(self):

        # 重置所有在静息态的神经元的当前步长
        self.cortex['current_step'][self.cortex['excite'] ==
                                    self.cortex['RP']] = 1
        ...
        self.vary_axon_inds.update(
            add_step_inds[self.cortex['current_step'][add_step_inds] ==
                          self.cortex['step_length'][add_step_inds]])

    def spike_axon(self):
        axon_end_slice = slice(self.cortex_static_part_slice.stop,
                               self.new_ind_start)
        ...
        to_nerves_mask[:] = (self.cortex['mother_type'][axon_end_slice]
                             == TYPE['axon']) * (self.cortex['is_active'][
                                 self.cortex['pre_ind'][axon_end_slice]] == 1)
        ...
        # 轴突末梢的状态更新后，与它有关的突触前轴突末端也要参与后续计算
    ...
    def get_axon_spike_excite(self, axon_inds, to_inds):
        transmitter_release_sum = self.cortex['transmitter_release_sum'][
            axon_inds]
        ...
        transmitter_release_sum[to_inds_has_size_mask] = self.cortex[
            POSTERIOR_SYNAPSE_EXINFO['size']][to_inds[to_inds_has_size_mask]]

        return self.cortex['tick_spike_times'][
            axon_inds] * transmitter_release_sum

    def spike_axon_end_loop(self):
        self.mark_will_recv_excite_with_nerve_inds(
            list(self.vary_axon_end_inds))
    ...
    def spike_axon_end(self):
        ...
        def filter_spike_inds(spike_inds):
            ...
            # 是否有上游兴奋
            will_no_recv_excite_inds_mask = self.cortex['will_recv_excite'][
                spike_inds] == 0
            will_recv_excite_spike_inds = spike_inds[
        ...
        in_RP_to_inds_mask = self.cortex['excite'][to_inds] == self.cortex[
            'RP'][to_inds]

        # 需要在模拟轴突传递兴奋给突触后目标前就重置突触后对象的excite
        self.reset_cortex_props_to_initial_state(reset_inds=to_inds,
                                                 include_props=['excite'])

        # 先传递轴突的兴奋给轴突末梢
        axon_spike_excite = self.get_axon_spike_excite(spike_axon_inds,
                                                       to_axon_end_inds)
        ...
        self.post_excite_2_each_cal_type_targ(spike_inds, to_inds,
                                              spike_excite)
        ...
        # 原本处于静息电位，在接收兴奋后低于静息电位的神经突，视为状态没改变，不参与后续计算
        ...
        # 更新状态变化的神经突inds
        self.vary_axon_end_inds = set(
            np.compress(self.cortex['type'][vary_to_inds] == TYPE['axon_end'],
                        vary_to_inds))
        self.vary_axon_end_inds.update(will_recv_excite_spike_inds)
        self.vary_dendrite_inds.update(
            np.compress(
                (self.cortex['type'][vary_to_inds] >= list(
                    DENDRITE_TYPE.values())[0]) *
                (self.cortex['type'][vary_to_inds] <= list(
                    DENDRITE_TYPE.values())[-1]),
                vary_to_inds,
            ))
        self.vary_soma_inds.update(
            np.compress(self.cortex['type'][vary_to_inds] == TYPE['soma'],
                        vary_to_inds))
        self.vary_axon_inds.update(
            np.compress(self.cortex['type'][vary_to_inds] == TYPE['axon'],
                        vary_to_inds))
    ...
    def update_refractory(self, spike_inds):
        ...
        is_need_refractory_mask[:] = (
            self.cortex['refractory'][spike_inds]
            == 1) * (self.cortex['tick_spike_times'][spike_inds] > 0)
        self.cortex['refractory'][spike_inds[is_need_refractory_mask]] = 2

        for release_type in ['excite', 'Fa', 'STP', 'STD', 'marker']:
            is_need_refractory_mask = (
                self.cortex['refractory'][spike_inds]
                == 1) * (self.cortex['tick_spike_times'][spike_inds] >
                         0) * (self.cortex[release_type][spike_inds] >
                               (PRE_LTP_SUM_BASE * .1))
            self.cortex['refractory'][spike_inds[is_need_refractory_mask]] = 2
    ...
    def mark_will_recv_excite_with_nerve_inds(self, spike_inds):
        ...
        self.cortex['will_recv_excite'][self.cortex['post_ind']
                                        [spike_inds]] = 1

    def get_spike_excite(self, spike_inds):

        # 基础脉冲兴奋
        base_excite = self.cortex['tick_spike_times'][
            spike_inds] * self.cortex['LTP'][spike_inds] * self.cortex[
                'transmitter_release_sum'][spike_inds]

        # 没有脉冲的细胞，取自发放兴奋作为脉冲兴奋
    ...
    def dendrite_self_synaptic(self):
        ...
        self_synaptic_dendrite_inds = (
            self.cortex['type'] >=
            dendrite_vals[0]) * (self.cortex['type'] <= dendrite_vals[-1]) * (
                self.cortex['self_synapse'] > 0)
        self.cortex['excite'][self_synaptic_dendrite_inds] = self.cortex[
            'excite'][self.cortex['soma_ind'][self_synaptic_dendrite_inds]]

    def spike_dendrite_loop(self):
        self.mark_will_recv_excite_with_nerve_inds(
            list(self.vary_dendrite_inds))
    ...
    def spike_dendrite(self):
        ...
        # # 步长不足的轴突末梢不传递兴奋
        # no_reach_step_length_mask = self.cortex['current_step'][
        #     spike_inds] < self.cortex['step_length'][spike_inds]
        # spike_excite[no_reach_step_length_mask] = self.cortex['RP'][
        #     spike_inds[no_reach_step_length_mask]]

        # # 树突棘传递的兴奋，需要加入棘头大小的因素
        # is_spine_mask = self.get_is_spine_mask(spike_inds)
        # spike_excite[is_spine_mask] *= self.cortex[SPINE_EXINFO['size']][
        #     spike_inds[is_spine_mask]] / SPINE_SIZE_SUM_LIMIT
        # self.cortex[SPINE_EXINFO['posted_excite']][
        #     spike_inds[is_spine_mask]] = spike_excite[is_spine_mask]
        ...
        self.vary_dendrite_inds = set(
            np.compress((self.cortex['type'][vary_to_inds] >= list(
                DENDRITE_TYPE.values())[0]) *
                        (self.cortex['type'][vary_to_inds] <= list(
                            DENDRITE_TYPE.values())[-1]), vary_to_inds))
    ...
    def post_excite_2_each_cal_type_targ(self, spike_inds, to_inds,
                                         spike_excite):
        spike_2_targ_mask = self.cortex['bool_util'][:len(spike_inds)]
        spike_2_targ_mask[:] = self.cortex['release_type'][
            spike_inds] == RELEASE_TYPE['excite']

        # max
        post_max_mask = spike_2_targ_mask * self.cortex['type'][
            to_inds] == TYPE['dendrite_max']
        self.post_excite(spike_inds[post_max_mask], to_inds[post_max_mask],
                         spike_excite[post_max_mask], 'max')
        # min
        post_min_mask = spike_2_targ_mask * self.cortex['type'][
            to_inds] == TYPE['dendrite_min']
        self.post_excite(spike_inds[post_min_mask], to_inds[post_min_mask],
                         spike_excite[post_min_mask], 'min')
        # multi
        post_multi_mask = spike_2_targ_mask * self.cortex['type'][
            to_inds] == TYPE['dendrite_multi']
        self.post_excite(spike_inds[post_multi_mask], to_inds[post_multi_mask],
                         spike_excite[post_multi_mask], 'multi')
        # add
        post_add_mask = spike_2_targ_mask * ~post_max_mask * ~post_min_mask * ~post_multi_mask
        spike_excite[post_add_mask] *= self.cortex['post_sign'][
            spike_inds[post_add_mask]]
        self.post_excite(spike_inds[post_add_mask], to_inds[post_add_mask],
                         spike_excite[post_add_mask], 'add')

    def post_excite(self, spike_inds, to_inds, excite, post_type='add'):
        if not len(spike_inds): return
        if post_type == 'add':
    ...
    def post_excite_add(self, spike_inds, to_inds, excite):
        ...
        if spike_nerve_type in [TYPE['axon_end'], *DENDRITE_TYPE.values()]:
            ...
            all_or_none_mask[:] = self.cortex['all_or_none'][
                spike_inds].astype(bool)
            np.add.at(self.cortex['excite'],
                      np.array(to_inds, int)[~all_or_none_mask],
                      excite.astype(int)[~all_or_none_mask])

            # 2. 再根据传递后的兴奋，来判断全或无的突触是否要执行全或无的兴奋传递
            posted_excite = excite + self.cortex['excite'][
                to_inds] - self.cortex['RP'][to_inds]
            all_or_none_can_post_mask = self.cortex[
                'bool_util'][:len(spike_inds)]
            all_or_none_can_post_mask[:] = (
                (np.sign(posted_excite)
                 == self.cortex['post_sign'][spike_inds]) |
            ...
            np.add.at(self.cortex['excite'], np.array(to_inds, int),
                      excite.astype(int))
    ...
    def post_excite_min(self, spike_inds, to_inds, excite):
        ''' 为了防止突触后目标本身兴奋就很低，导致求minumum无法传递兴奋
            在传递前将它们的兴奋设置为最大值，就可以让最终兴奋为突触前的兴奋中的最小值
        '''
        ...
        np.minimum.at(self.cortex['excite'], to_inds,
                      excite + self.cortex['RP'][to_inds])
    ...
    def update_max_circuit_length(self):
        posterior_synapse_slice = self.get_posterior_synapse_slice()
        alive_posterior_synapse_inds = self.cortex['ind'][
            posterior_synapse_slice][
                self.cortex['LTP'][posterior_synapse_slice] > 0]
        np.maximum.at(
            self.cortex['max_circuit_length'],
            self.cortex[POSTERIOR_SYNAPSE_EXINFO['spine_ind']]
            [alive_posterior_synapse_inds].astype(int),
    ...
    def axon_end_post_spine_active(self, spike_inds, spike_excite):
        ...
        # )
        spine_inds = self.cortex[
            POSTERIOR_SYNAPSE_EXINFO['spine_ind']][spike_inds].astype(int)
        self.cortex['spine_active'][spine_inds[self.cortex['spine_active']
                                               [spine_inds] == -1]] = np.inf
        np.minimum.at(
            self.cortex['spine_active'],
            spine_inds,
    ...
    def get_post_spine_active_value(self, spike_inds):
        ...
        spine_active_ratio = self.get_spine_active_ratio_with_excite_and_LTP(
            spike_inds)
        post_spine_active_value = self.cortex['LTP'][
            spike_inds] * spine_active_ratio
    ...
    def get_spine_active_ratio_with_excite_and_LTP(self, spike_inds):
        ratio_with_full_excite = (
            ...
            self.cortex['RP'][spike_inds]) / ABSTRACT_EXCITE
        ratio_with_max_LTP = self.cortex['LTP'][spike_inds] / self.cortex[
            'max_pre_synapse_LTP'][self.cortex['post_ind'][spike_inds]]
        return np.minimum(1, ratio_with_full_excite / ratio_with_max_LTP)

    def axon_end_post_Fa(self, spike_inds, spike_excite):
        release_type_is_Fa_mask = self.cortex['release_type'][
            spike_inds] == RELEASE_TYPE['Fa']
        to_inds = self.cortex['post_ind'][spike_inds[release_type_is_Fa_mask]]
        np.maximum.at(self.cortex['Fa'], to_inds,
                      spike_excite[release_type_is_Fa_mask])
    ...
    def axon_end_post_anti_Fa(self, spike_inds, spike_excite):
        ...
        release_type_is_anti_Fa_mask = self.cortex['release_type'][
            spike_inds] == RELEASE_TYPE['anti_Fa']
        to_inds = self.cortex['post_ind'][
            spike_inds[release_type_is_anti_Fa_mask]]

        # self.reset_calculate_props(to_inds, 'anti_Fa')

        np.maximum.at(self.cortex['anti_Fa'], to_inds,
                      spike_excite[release_type_is_anti_Fa_mask])

    def axon_end_post_STD(self, spike_inds, spike_excite):
        release_type_is_STD_mask = self.cortex['bool_util'][:len(spike_inds)]
        release_type_is_STD_mask[:] = self.cortex['release_type'][
            spike_inds] == RELEASE_TYPE['STD']
        to_inds = self.cortex['post_ind'][spike_inds[release_type_is_STD_mask]]
        np.add.at(self.cortex['STD'], to_inds,
                  spike_excite[release_type_is_STD_mask])
    ...
    def posterior_synapse_post_excite_to_spine(self, spike_inds):
        ...
        spine_inds = self.cortex[
            POSTERIOR_SYNAPSE_EXINFO['spine_ind']][spike_inds].astype(int)
        self.cortex[SPINE_EXINFO['min_excite']][spine_inds[self.cortex[
            SPINE_EXINFO['min_excite']][spine_inds] == -np.inf]] = np.inf
        np.minimum.at(self.cortex[SPINE_EXINFO['min_excite']], spine_inds,
                      self.cortex['excite'][spike_inds])

    def axon_end_post_STP(self, spike_inds, spike_excite):
        release_type_is_STD_mask = self.cortex['release_type'][
            spike_inds] == RELEASE_TYPE['STP']
        to_inds = self.cortex['post_ind'][spike_inds[release_type_is_STD_mask]]
        np.maximum.at(self.cortex['STP'], to_inds,
                      spike_excite[release_type_is_STD_mask])
    ...
    def post_marker(self, spike_inds, spike_excite):
        ...
        release_type_is_marker_mask[:] = (
            self.cortex['release_type'][spike_inds]
            == RELEASE_TYPE['marker']) * (self.cortex['is_active'][spike_inds]
                                          == 0
                                          )  # 没活跃过的的突触才能传递marker，保证一帧里只传递一次
        np.add.at(
            self.cortex['marker_remain'],
        ...
        release_type_is_marker_multi_mask = self.cortex[
            'bool_util'][:len(spike_inds)]
        release_type_is_marker_multi_mask[:] = (
            self.cortex['release_type'][spike_inds]
            == RELEASE_TYPE['marker_multi']) * (
                self.cortex['is_active'][spike_inds] == 0
            )  # 没活跃过的的突触才能传递marker，保证一帧里只传递一次
        np.maximum.at(
            self.cortex['marker_remain'],
            self.cortex['post_ind'][
                spike_inds[release_type_is_marker_multi_mask]],
            1,
        )
        np.multiply.at(
            self.cortex['marker_remain'],
    ...
    def weaken_synapse(self):
        if CORTEX_OPTS['enable_posterior_form'] == 0: return

        self.weaken_synapse_by_post_synaptic_resource_limitation()

        # 弱化后，需要标记LTP降为0的突触和相关突触为死亡状态
    ...
    def weaken_synapse_by_post_synaptic_resource_limitation(self):
        ...
        # 重新计算所有后验突触的突触后膜上的LTP总量，总量超限的突触后膜的那些突触就是需要被弱化的突触
        ...
        # )
        each_post_synapse_resource_sum = np.full(len(weaken_synapse_inds),
                                                 POST_SYNAPSE_RESOURCE_SUM)
        weaken_inds_mask = cortex['post_synapse_LTP_sum'][cortex['post_ind'][
            weaken_synapse_inds]].astype(int) > each_post_synapse_resource_sum
    ...
    def adjust_LTP_limit_of_each_stable_circuit(
        ...
        resource_limit_of_each_synapse = self.get_resource_limit_of_synapse(
            to_adjust_synapse_inds
        ) if resource_limit_of_each_synapse == None else np.full(
            len(to_adjust_synapse_inds), resource_limit_of_each_synapse)

        cortex['LTP'][
            to_adjust_synapse_inds] *= resource_limit_of_each_synapse / self.cortex[
                'post_synapse_LTP_sum'][self.cortex['post_ind']
                                        [to_adjust_synapse_inds]]

    def get_resource_limit_of_synapse(self, synapse_inds):
        return self.get_resource_limit_of_spine(self.cortex[
            POSTERIOR_SYNAPSE_EXINFO['spine_ind']][synapse_inds].astype(int))
    ...
    def get_resource_limit_of_spine(self, spine_inds):
        ...
        return np.power(
            STANDARD_PREDICT_EXCITE, 1 / max_circuit_length
        ) / POSTERIOR_SYNAPSE_TRANSMITTER_SUM * POST_SYNAPSE_RESOURCE_SUM

    # def get_spine_resource_limit_of_each_synapse(self, spine_inds):
    #     stable_spine_mask = self.get_stable_spine_mask(
    #         spine_inds)
    #     spine_resource_limit_of_each_synapse = np.full(
    #         len(spine_inds), POST_SYNAPSE_RESOURCE_SUM)
    #     spine_resource_limit_of_each_synapse[
    #         stable_spine_mask] = np.power(
    #             STANDARD_PREDICT_EXCITE, 1 /
    #             (self.cortex['max_circuit_length'][
    #                 spine_inds[stable_spine_mask]])
    #         ) / POSTERIOR_SYNAPSE_TRANSMITTER_SUM * POST_SYNAPSE_RESOURCE_SUM
    #     return spine_resource_limit_of_each_synapse

    # def adjust_synapse_with_post_synapse_LTP_sum_lower_than_LTP_limit_of_has_marker_remain_spine(
    #         self):
    #     self.update_post_synapse_LTP_sum()
    #     to_adjust_synapse_inds = self.get_all_synapse_inds_on_spines(
    #         self.get_stable_spine_inds_of_nowa_num())
    ...
    def get_synapse_inds_on_active_and_stable_spines(self):
        ...
        has_marker_remain_spine_inds_of_nowa_num = self.get_has_marker_remain_spine_inds_of_nowa_num(
        )

        #
        stable_spine_inds = has_marker_remain_spine_inds_of_nowa_num[
            self.get_stable_spine_mask(
        ...
        return cortex['ind'][posterior_synapse_slice][
            stable_spine_mask[cortex[POSTERIOR_SYNAPSE_EXINFO['spine_ind']]
    ...
    def get_synapse_inds_on_stable_spines_of_nowa_num(self):
        ...
        return self.get_all_synapse_inds_on_spines(spine_inds_of_nowa_num[
            self.get_stable_spine_mask(spine_inds_of_nowa_num)])

    def get_stable_spine_inds_of_nowa_num(self):
        spine_inds_of_nowa_num = self.get_spine_inds_of_nowa_num()
        return spine_inds_of_nowa_num[self.get_stable_spine_mask(
            spine_inds_of_nowa_num)]
    ...
    def get_stable_spine_mask(self, spine_inds):
        ...
        np.add.at(
            ...
            self.cortex[POSTERIOR_SYNAPSE_EXINFO['spine_ind']]
            [posterior_synapse_ind].astype(int),
            1,
        )
        self.cortex['ind'][posterior_synapse_slice][
            self.cortex[POSTERIOR_SYNAPSE_EXINFO['spine_ind']]
            [posterior_synapse_slice].astype(int) == 4779]
        return (synapse_sum_on_spines[spine_inds]
    ...
    def get_active_and_stable_spine_inds_of_nowa_num(self):
        ...
        return active_spine_inds_of_nowa_num[self.get_stable_spine_mask(
            active_spine_inds_of_nowa_num)]

    def get_active_and_stable_spine_inds_out_of_nowa_num(self):
        all_spine_inds_out_of_nowa_num = self.get_all_spine_inds([
            num for num in np.arange(10)
            if num != self.controller.get_active_num()
        ])
    ...
    def get_active_and_stable_spine_inds_of_all_num(self):
        ...
        return active_spine_inds_of_all_num[self.get_stable_spine_mask(
            active_spine_inds_of_all_num)]
    ...
    def get_max_LTP_link_on_spine(self,
                                  spine_ind,
                                  synapse_inds_on_this_spine=[]):
        synapse_inds_on_this_spine = synapse_inds_on_this_spine if len(
            synapse_inds_on_this_spine
        ...
        if len(pre_synapse_inds) == 0:
            ...
            pre_synapse_ind = pre_synapse_inds[np.argmax(
                self.cortex['LTP'][pre_synapse_inds])]

        return [
            pre_synapse_ind, *self.get_max_LTP_link_on_spine(
                pre_synapse_ind, synapse_inds_on_this_spine)
        ] if pre_synapse_ind != None else []
    ...
    def get_all_synapse_inds_on_spines(self, spine_inds):
        ...
        all_synapse_inds_on_spines = self.cortex['ind'][spine_mask[self.cortex[
            POSTERIOR_SYNAPSE_EXINFO['spine_ind']].astype(int)]]
    ...
    def mark_died_synapse(self):
        cortex = self.cortex
        posterior_synapse_slice = slice(self.cortex_static_nerve_slice.stop,
                                        self.new_ind_start)

        died_synapse_ind = cortex['ind'][posterior_synapse_slice][(
            cortex['LTP'][posterior_synapse_slice] != 0
        ) * (
            # 自身的LTP小于阈值且不为0，则需要置为0
            (cortex['LTP'][posterior_synapse_slice] < DIED_LTP_THRESHOLD)
            |
            # 或者自身LTP不为0，但是突触后目标也是突触，且LTP为0（已死），
            # 说明自己已经失去了有效的连接对象，则需要将自身LTP也置为0
            ((cortex['is_synapse'][cortex['post_ind'][posterior_synapse_slice]]
              != 0) *
    ...
    def reduce_excite_with_tick_spike_times(self, reduce_inds, spike_times=[]):
        ...
        spike_times = self.cortex['tick_spike_times'][
            can_reduce_inds] if not len(spike_times) else spike_times
        self.cortex['excite'][can_reduce_inds] -= spike_times * -self.cortex[
            'RP'][can_reduce_inds]
    ...
    def get_excite_inds_by_type(self, part_type):
        if isinstance(part_type, int): part_type = [part_type]
        # 只要大于最低同步电位，就发放脉冲
        if len(part_type) > 1:
            return np.compress((self.cortex['type'] >= part_type[0]) *
    ...
    def leak_nerves_excite_to_rest_potential(self):
        ...
        leak_mask[:] = (self.cortex['excite'][cortex_slice] <
                        ATP) * (self.cortex['self_synapse'][cortex_slice] == 0)
        self.cortex['excite'][cortex_slice][leak_mask] = self.cortex['RP'][
            cortex_slice][leak_mask]

    def set_spike_times_with_excite(self, inds):
        self.set(
            inds, 'tick_spike_times',
            np.maximum(
                0,
                np.round((self.get(inds, 'excite') - self.get(inds, 'RP')) /
                         abs(self.get(inds, 'RP'))),
            ))

        # 根据计算后的tick_spike_times，重新设置excite，过滤掉一些可以被四舍五入的兴奋
        self.cortex['excite'][inds] = self.cortex['tick_spike_times'][
            inds] * abs(self.cortex['RP'][inds]) + self.get(inds, 'RP')

        # 记录历史最大脉冲次数
        np.maximum.at(self.cortex['history_tick_spike_times'], inds,
    ...
    def get_write_cortex_info(self):
        info = {
            'region': {info['region_no']: info
                       for info in REGION.values()},
            'nerve_props_keys_map':
            PART_PROPS_KEYS_MAP,
            'cortex_static_part_slice_stop':
            int(self.cortex_static_part_slice.stop),
            'cortex_static_nerves_slice_stop':
            int(self.cortex_static_nerve_slice.stop),
            'cortex_slice_stop':
            int(self.new_ind_start),
            'nerve_marker_map':
            self.nerve_marker_map,
        }
        return info
    ...
    def get_save_posterior_inds(self):
        return np.concatenate((self.get_all_spine_inds(),
                               np.arange(self.cortex_static_nerve_slice.stop,
                                         self.new_ind_start)))
    ...
    def write_cortex(self, desc=""):
        ...
        if is_save:
            ...
            write_slice = CORTEX_OPTS['write_slice_lambda'](self, self.cortex,
                                                            get_soma_inds,
                                                            REGION, TYPE)
        cortex_info[
            'write_slice_static_ind_stop'] = self.cortex_static_nerve_slice.stop
        cortex_info['write_slice_length'] = write_slice.stop - (
            write_slice.start or 0) if isinstance(write_slice,
                                                  slice) else len(write_slice)
        ...
        write_n_read_cortex.write_cortex(self,
                                         cortex_info,
                                         cortex_matrix_map,
                                         desc=desc,
                                         write_mode=write_mode,
                                         suffix=self.mock_file_name
                                         or self.mnist_name)
    ...
    def get_soma_name(self, soma_ind):
        region = [
            region for region in REGION.values()
            if region['region_no'] == self.cortex['region_no'][soma_ind]
        ][0]
        neuron = [
            neuron for neuron in region['neurons'].values()
