...
class FormNerve():
    ...
    def spring_nerve_packs_by_marker(self,
        ...
        # 如果强化突触后，同一位点上存在一组或多组LTP完全一样的突触，将每组这样的突触的第一个突触的LTP加1，强制赋予它微弱的竞争优势
    ...
    def add_LTP_with_form_and_firm_nerve_inds(self, cortex_obj,
        ...
        # ''' TODO 如果一个位点上已经存在较强的突触，对于其他突触来说，获得WTA的几率更小，应该把更多突触前资源分配
        #     到对其他竞争更小的点位上，这对于每个细胞来说是更聪明的做法，整体而言也可以加快突触链路建立的速度
        # '''
        # # / np.maximum(
        # #     np.power(
        # #         (max_pre_synapse_LTP_on_post_synapse_nerve -
