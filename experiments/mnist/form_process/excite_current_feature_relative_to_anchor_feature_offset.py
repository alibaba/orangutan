    ...
    for x_or_y in range(VISUAL_FIELD_WH[0]):
        for x_or_y_delta in range(-VISUAL_FIELD_WH[0], VISUAL_FIELD_WH[0] + 1):
            if not (x_or_y + x_or_y_delta) in range(VISUAL_FIELD_WH[0]):
                continue
            mother_inds.extend(get_soma_inds(f"anchor_point_feature_position", f"{x_or_y}"))
            father_inds.extend(
                get_soma_inds("offset_of_current_feature_position_relative_to_anchor_feature", f"{x_or_y_delta}")
            )
...
def prohibit_excite_offset(cortex_obj):
    ...
    for x_or_y in range(VISUAL_FIELD_WH[0]):
        for x_or_y_delta in range(-VISUAL_FIELD_WH[0], VISUAL_FIELD_WH[0] + 1):
            if not (x_or_y + x_or_y_delta) in range(VISUAL_FIELD_WH[0]):
                continue
            mother_inds.extend(get_soma_inds(f"anchor_point_feature_position", f"{x_or_y}_A_inhibit"))
    ...
    for x_or_y in range(VISUAL_FIELD_WH[0]):
        for x_or_y_delta in range(-VISUAL_FIELD_WH[0], VISUAL_FIELD_WH[0] + 1):
            if not (x_or_y + x_or_y_delta) in range(VISUAL_FIELD_WH[0]):
                continue
            mother_inds.extend(
                get_soma_inds(f"current_feature_position", f"{x_or_y+x_or_y_delta}_A_inhibit")
            )
