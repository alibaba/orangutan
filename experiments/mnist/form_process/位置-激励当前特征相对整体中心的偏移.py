    ...
    for center_x_or_y in range(VISUAL_FIELD_WH[0]):
        for x_or_y_delta in range(-VISUAL_FIELD_WH[0], VISUAL_FIELD_WH[0] + 1):
            if not (center_x_or_y + x_or_y_delta) in range(VISUAL_FIELD_WH[0]):
                continue
            mother_inds.extend(get_soma_inds(f"位置_整体中心", f"{center_x_or_y}"))
            father_inds.extend(
                get_soma_inds("位置_当前特征相对整体中心的偏移", f"{x_or_y_delta}")
            )
    ...
    for center_x_or_y in range(VISUAL_FIELD_WH[0]):
        for x_or_y_delta in range(-VISUAL_FIELD_WH[0], VISUAL_FIELD_WH[0] + 1):
            if not (center_x_or_y + x_or_y_delta) in range(VISUAL_FIELD_WH[0]):
                continue
            mother_inds.extend(
                get_soma_inds(f"位置_整体中心", f"{center_x_or_y}_A抑制")
            )
    ...
    for center_x_or_y in range(VISUAL_FIELD_WH[0]):
        for x_or_y_delta in range(-VISUAL_FIELD_WH[0], VISUAL_FIELD_WH[0] + 1):
            if not (center_x_or_y + x_or_y_delta) in range(VISUAL_FIELD_WH[0]):
                continue
            mother_inds.extend(
                get_soma_inds(f"位置_当前特征", f"{center_x_or_y+x_or_y_delta}_A抑制")
            )
