...
def get_soma_inds(
    ...
    例子：
        调用: get_soma_inds("点", f"45.0方向的边缘点")
        Returns: array([2418, 2419, 2420, 2421, 2422, ... 3197, 3198, 3199, 3200, 3201])

    Parameters:
        region_name (str): 脑区名称
        neuron_names (str): 神经元房室名称
        hyper_col_inds_in_region (str|int): 位于该脑区内的超柱的索引 default("all_hyper_col")
        mini_col_inds_in_hyper_col (str|int): 位于每个超柱内的微柱的索引 default("all_mini_col")
...
def get_around_and_center_hyper_col_inds_with_around_mask(
    ...
    Returns:
        around_pos_inds (numpy.array): 位于外周感受野的皮质超柱在脑区内的索引
        center_pos_inds (numpy.array): 位于中央感受野的皮质超柱在脑区内的索引
        inrange_around_pos_mask (numpy.array): 外周感受野的皮质超柱的索引在脑区范围([0,该脑区的皮质超柱的总量])内，进而可以正常与中央感受野的皮质超柱建立突触的掩码
    """
...
def get_popu_props(prop_type, prop_value, popu_range=4):
    ...
    if not is_can_popu or prop_value == "无":
...
def get_orient_distance_matrix(orient, matrix_wh, orient_range):
    """
    获得一个神经元与其周边的每个神经元的相对方位和一个特定方位的近似比率

    伪代码：
        1. 创建一个宽高相等且为奇数的矩阵
        2. 以矩阵中心作为原点建立一个Y轴向下、X轴向右的直角坐标系并将其映射到一个方位角系统（向量(x=0,y=-1)对应于360度或0度方位角）
        3. 计算每个向量(x,y)的方位角与给定方位的偏移量绝对值
